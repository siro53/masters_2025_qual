...
int main() {
...

    {
        ...
        while(!que.empty()) {
            ...
            REP(dir, 4) {
                ...
                if(C[ni][nj] == '.' or C[ni][nj] == 'a') {
                    if(chmin(dp[ni][nj], dp[i][j] + 3)) {
                        que.emplace(dp[ni][nj], ni, nj);
                        pre[ni][nj] = {i, j};
                    }
                } else if(C[ni][nj] == '@') {
                    int cost = (si == ni or sj == nj ? 4 : 10);
                    if(chmin(dp[ni][nj], dp[i][j] + cost)) {
                        que.emplace(dp[ni][nj], ni, nj);
                        pre[ni][nj] = {i, j};
                    }
                }
        ...
        REP(i, N) REP(j, N) if(C[i][j] == 'a' and dp[i][j] > 0) {
            auto path = restore_bfs(i, j, pre);
            for(auto [ii, jj] : path) if(C[ii][jj] == '@') C[ii][jj] = 'a';
        }
    }
    ...
    auto del_ore = [&]() {
        ...
        REP(k, SZ(starts)) {
            ...
            // for(auto [i, j, cc] : goals) {
            //     int mn = INF;
            //     REP(x, N) REP(y, N) {
            //         if(x == ssi and y == ssj) continue;
            //         if(C[x][y] == 'a') {
            //             chmin(mn, abs(i - x) + abs(j - y));
            //         }
            //     }
            //     next_cost.push_back(mn == INF ? 0 : mn);
            // }
            // assert(SZ(goals) == SZ(next_cost));
